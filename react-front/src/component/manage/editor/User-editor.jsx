    ...
    render() {
        return (
            <Modal visible={this.state.visible} width="1200px"
                ...
                <WrappedUserEditor codeList={this.props.codeList} item={this.state.item} {...this.childMethod} />
            </Modal>
        );
    }
}
...
const UserEditorForm = (props) => {
    ...
    let method = {
        ...
        // },
        renderOpts: () => {
            return props.codeList.map((it) => <Select.Option key={it.id} value={it.name}>{it.name}</Select.Option>)
        },
